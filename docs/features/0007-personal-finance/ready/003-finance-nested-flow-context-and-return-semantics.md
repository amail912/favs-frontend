...
## Outcome
Finance create and detail flows open as local modal or sheet overlays on top of the current finance route, preserve the active finance workspace context while open, and close with one consistent return and browser-history contract.

## In Scope
- Nested finance create flow opened from finance surfaces.
- Nested finance transaction-detail flow opened from finance surfaces.
...
- Precedes `009`, `010`, and `011` because create and detail stories should reuse one return contract instead of inventing their own.
...
- Scope boundary:
  - this story defines the shared contract only; later finance stories must reuse it instead of redefining return semantics locally
...
- Opening a finance detail flow from a finance surface preserves the active underlying finance route while the overlay is open.
...
- Integration:
  ...
  - detail overlay open and close preserves underlying finance surface
...
- E2E:
  ...
  - open finance detail, close it, verify user remains in the same finance workspace
  - nested finance flow return semantics stay coherent on both desktop-like and mobile-like overlay presentation
